...
select customerName from customers where country = "USA" && city = "NYC" && creditLimit < 20000;
select productCode, productName, buyPrice from products where buyPrice <= 50 || buyPrice >= 100;
select employeeNumber, lastName, firstName from employees where lastName like '%am%' && firstName like '%er';
select contactLastName, contactFirstName from customers order by contactLastName DESC, contactFirstName ASC;
select productName, buyPrice from products order by buyPrice desc limit 0, 10;
select productName, (select priceEach from orderdetails order by priceEach ASC limit 1) from products where productCode = (select productCode from orderdetails order by priceEach ASC limit 1);
select city from customers where customerNumber in(select customerNumber from payments where amount >= 40000);
