...
VALUES(1901, 'hong', 'jin', 'x5000', 'nike@google.com', '7', null, 'Research');
-- select * from employees;

-- 2. Jobtitle이 research인 직원이 속한 office의 city를 Cheonan-si으로 변경하시오
update offices inner join employees on employees.officeCode = offices.officeCode 
set offices.city = 'Cheonan-si' where employees.jobTitle = 'Research'; 
-- select * from offices;
...
create table employees(
   ...
   performance INT(11) DEFAULT NULL,
    ...
    PRIMARY KEY (emp_id), CONSTRAINT fk_performance 
...
('Mary Doe', 1, 50000), ('Cindy Smith', 3, 65000), ('Sue Greenspan', 4, 75000), ('Grace Dell', 5, 125000), 
('Nancy Johnson', 3, 85000), ('John Doe', 2, 45000), ('Lily Bush', 3, 55000);
select * from merit;
select * from employees;

/*
4. 실적 별 월급은 실적이 올라갈 때 마다 월급=월급+월급
*percentage로 오른다고 가정하고, 현재 employees 테이블에 들어
가 있는 월급 데이터는 실적정보가 반영되어 있지 않다고 할 때 실
적을 반영한 월급으로 갱신 시키시오
*/
update employees inner join merit on employees.performance = merit.performance set salary = salary + salary * percentage;
select * from employees;

-- 5. Employees와 offices Table에서 officeCode가 1인 Row들을 삭제 하시오 (삭제 후 결과 화면 스크린 샷)
use `classicmodels`;
